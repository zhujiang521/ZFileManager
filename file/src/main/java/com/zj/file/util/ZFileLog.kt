...
import com.zj.file.content.LOG_TAG
...
object ZFileLog {
    ...
    private val packageName = ZFileLog::class.java.name
    ...
    @JvmStatic
    fun v(msg: String? = "") {
        if ((type and TYPE_V != 0 && getZFileConfig().showLog) or Log.isLoggable(
                APP_TAG,
                Log.VERBOSE
            )
        ) {
    ...
    @JvmStatic
    fun d(msg: String? = "") {
        if ((type and TYPE_D != 0 && getZFileConfig().showLog) or Log.isLoggable(
                APP_TAG,
                Log.DEBUG
            )
        ) {
    ...
    @JvmStatic
    fun i(msg: String? = "") {
        if ((type and TYPE_I != 0 && getZFileConfig().showLog) or Log.isLoggable(
                APP_TAG,
                Log.INFO
            )
        ) {
    ...
    @JvmStatic
    fun w(msg: String? = "") {
        if ((type and TYPE_W != 0 && getZFileConfig().showLog) or Log.isLoggable(
                APP_TAG,
                Log.WARN
            )
        ) {
    ...
    @JvmStatic
    fun e(msg: String? = "") {
        if ((type and TYPE_E != 0 && getZFileConfig().showLog) or Log.isLoggable(
                APP_TAG,
                Log.ERROR
            )
        ) {
